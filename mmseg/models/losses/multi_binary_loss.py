...
def _make_one_hot(gt, num_classes, ignore=(255, )):
    """
...
def binary_loss(preds, labels, loss_func, num_classes, class_weight=None, class_weight_norm=False, smooth=1):
...
def multi_loss(pred_raw,
    ...
    for b in range(batch):
        if b == 0:
            ...
            labels = label[b:b + 1, 0:1]
            loss += binary_loss(preds, labels, loss_func, num_classes, class_weight, class_weight_norm, smooth)
        elif b == 1:
            num_classes = 2
            preds = pred[b:b + 1, 1:3]
            labels = label[b:b + 1, 1:3]
            loss += binary_loss(preds, labels, loss_func, num_classes, class_weight, class_weight_norm, smooth)
        else:
            num_classes = 4
            preds = pred[b:b + 1, 3:7]
            labels = label[b:b + 1, 3:7]
            loss += binary_loss(preds, labels, loss_func, num_classes, class_weight, class_weight_norm, smooth)
