...
from mmseg.models.data_parallel import ModelMMDistributedDataParallel
...
def get_module(module):
    ...
    """
    if isinstance(module, MMDistributedDataParallel) or isinstance(module, ModelMMDistributedDataParallel):
...
class MultiTaskMeanTeacher(BaseSegmentor):
    ...
    #                optim_wrapper: OptimWrapper, ddp_reducer=None) -> Dict[str, torch.Tensor]:
    #     # Enable automatic mixed precision training context.
    #     self.ddp_reducer = ddp_reducer
    ...
    #     self.ddp_reducer = None
    #     # optim_wrapper.update_params(parsed_losses)
    #     return log_vars

    # def generete_pseudo_weight(self, ema_logits, dev, pseudo_threshold):
    ...
    #     ps_large_p = pseudo_prob.ge(pseudo_threshold).long() == 1
    #     b, h, w = pseudo_label.shape
    #     ps_size = torch.full((b, ), h * w) - torch.sum(mask, dim=(1, 2))
    #     ps_size[ps_size==0] = 1
    #     pseudo_weight = torch.sum(ps_large_p, dim=(1, 2)).cpu() / ps_size
    #     pseudo_weight[ps_size==0] = 1e-5
    #     pseudo_weight[pseudo_weight==0] = 1e-5
    #     # pseudo_weight = 1.0

    #     return pseudo_weight.to(dev)
    ...
    def semi_loss(self, inputs: Tensor, data_samples: SampleList) -> dict:
        ...
        factor = self.local_iter / self.max_iters
        ...
        mask_vessel_weight = self.generete_pseudo_weight(ema_logits[:, 0:1], dev, self.pseudo_threshold)

        mask_odoc = torch.ones((B, H, W), device=dev, dtype=torch.long) * 255
        mask_odoc[ema_logits[:, 1] > 0.5] = 1
        mask_odoc[ema_logits[:, 2] > 0.5] = 2
        mask_odoc_weight = self.generete_pseudo_weight(ema_logits[:, 1:3], dev, self.pseudo_threshold)
        ...
        mask_lesion_weight = self.generete_pseudo_weight(ema_logits[:, 3:7], dev, self.pseudo_threshold)
        ...
        self.get_model().decode_head.semi = True
        semi_losses = self.get_model().loss(augmented_inputs, data_samples, True)
        semi_losses = add_prefix(semi_losses, "semi")
        return semi_losses
    ...
    def loss(self, inputs: Tensor, data_samples: SampleList) -> dict:
        ...
        """
        self.get_model().decode_head.semi = False
