...
class MultiTaskMask2FormerHeadLearnGenerateQuery(MMDET_Mask2FormerHead):
    ...
    def __init__(self,
        ...
        self.query_embed = nn.Embedding(self.num_queries * 2, feat_channels)
    ...
    def _loss_by_feat_single(self, cls_scores: Tensor, mask_preds: Tensor,
        ...
        if not self.semi:
            ...
            #shape (batch_size, num_queries)
            n1 = num_query_per_task
            n2 = num_query_per_task * 2
            n3 = num_query_per_task * 3
            labels_list[0][n2:n3] = labels_list[1][n2:n3]
            label_weights_list[0][n2:n3] = label_weights_list[1][n2:n3]
            mask_weights_list[0][n2:n3] = mask_weights_list[1][n2:n3]

            labels_list[2][n2:n3] = labels_list[3][n2:n3]
            label_weights_list[2][n2:n3] = label_weights_list[3][n2:n3]
            mask_weights_list[2][n2:n3] = mask_weights_list[3][n2:n3]

            labels_list[4][n1:n2] = labels_list[5][n1:n2]
            label_weights_list[4][n1:n2] = label_weights_list[5][n1:n2]
            mask_weights_list[4][n1:n2] = mask_weights_list[5][n1:n2]
    ...
    def _forward_head(self, decoder_out: Tensor, mask_feature: Tensor,
        ...
        mask_embed = []
        for i in range(3):
            mask_embed.append(self.mask_embed[i](decoder_out[:, self.num_queries+i*num_query_per_task:self.num_queries+(i+1)*num_query_per_task]))
    ...
    def forward(self, x: List[Tensor],
        ...
        # query_feat = torch.cat([learn_query_feat, generate_query_feat], dim=0)
        # cls_pred, mask_pred, attn_mask = self._forward_head(
        #     query_feat, mask_features, multi_scale_memorys[0].shape[-2:])
        # cls_pred_list.append(cls_pred)
        # mask_pred_list.append(mask_pred)
        generate_query_feat = generate_query_feat.detach()
        ...
        self_attn_mask = torch.ones((batch_size * self.num_heads, self.num_queries * 2, self.num_queries * 2),
                                     device=attn_mask.device)
        self_attn_mask[:, :self.num_queries, :self.num_queries] = 0
        self_attn_mask[:, self.num_queries:, self.num_queries:] = 0
        # num_query_per_task = self.num_queries // 3
        # for i in range(6):
        #     self_attn_mask[:, i*num_query_per_task:(i+1)*num_query_per_task, i*num_query_per_task:(i+1)*num_query_per_task] = 1
        ...
        for i in range(self.num_transformer_decoder_layers):
            ...
            cls_attn_mask = attn_mask.new_ones(attn_mask.shape)
            cls_attn_mask = cls_attn_mask < 0.5

            cross_attn_mask = torch.cat([cls_attn_mask, attn_mask], dim=1)
