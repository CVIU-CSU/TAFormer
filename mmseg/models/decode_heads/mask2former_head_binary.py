...
class BinaryMask2FormerHead(MMDET_Mask2FormerHead):
    ...
    def loss(self, x: Tuple[Tensor], batch_data_samples: SampleList,
        ...
        # forward
        all_mask_preds = self(x, batch_data_samples[:x[0].shape[0]])
        # loss
        losses = self.loss_by_feat(all_mask_preds,
    ...
    def get_targets(
        ...
        (mask_targets_list, mask_weights_list, avg_factor_list) = results

        avg_factor = sum(avg_factor_list)

        res = (mask_targets_list,mask_weights_list, avg_factor)

        return res

    def _get_targets_single(self, mask_pred: Tensor,
        ...
        mask_weights[gt_labels] = pseudo_weight
        avg_factor = num_gts

        return (mask_targets, mask_weights, avg_factor)
    ...
    def loss_by_feat(self, all_mask_preds: Tensor,
        ...
        num_dec_layer = 0
        for loss_mask_i, loss_dice_i in zip(
    ...
    def _loss_by_feat_single(self, mask_preds: Tensor,
                             batch_gt_instances: List[InstanceData],
                             batch_img_metas: List[dict],
        ...
        avg_factor) = self.get_targets(mask_preds_list,
                                    batch_gt_instances, batch_img_metas, batch_pseudo_weight)
        ...
        num_total_masks = reduce_mean(mask_preds.new_tensor([avg_factor]))
    ...
    def _forward_head(self, decoder_out: Tensor, mask_feature: Tensor,
        ...
        for i in range(self.num_queries):
            mask_embed.append(self.mask_embed[i](decoder_out[:, i:(i+1)]))
